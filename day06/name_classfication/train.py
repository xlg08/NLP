...
def train_rnn():
    ...
    epochs = 2
    # 2.数据
    x_list, y_list = read_data('data/name_classfication1.txt')
    dataset = NameClassDataset(x_list, y_list, n_letters)
    # 3.模型
    rnn = RNN(input_size=n_letters, hidden_size=64, output_size=n_categories,batch_size=batch_size)
    # 4.优化器
    ...
    for epoch in range(epochs):
        ...
        for i, (x_batch, y_batch) in enumerate(dataloader):
            ...
            i_predict_tag=sum(torch.argmax(output, dim=-1) == y_batch)
...
def train_lstm():
    ...
    dataset = NameClassDataset(x_list, y_list, n_letters)
    # 3.模型
    lstm = LSTM(input_size=n_letters, hidden_size=64, output_size=n_categories,batch_size=batch_size)
    # 4.优化器
...
def train_gru():
    ...
    dataset = NameClassDataset(x_list, y_list, n_letters)
    # 3.模型
    gru = GRU(input_size=n_letters, hidden_size=64, output_size=n_categories,batch_size=batch_size)
    # 4.优化器

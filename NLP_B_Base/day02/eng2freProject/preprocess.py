...
def my_getdata():
    ...
    # 定义两个空列表
    # tmp_pair 存储的是 [英文句子, 法文句子] 缓存数据
    # my_pairs 存储的是 [[英文句子1, 法文句子1], [英文句子2, 法文句子2], ...]  形式的处理后的样本数据
    tmp_pair, my_pairs = [], []
    # 循环遍历my_lines
    ...
    for line in my_lines:
        ...
        for item in line.split('\t'):  # line.split() 方法返回列表的形式
            # print('item--->', item)
            # 将每行样本中的英文句子和法文句子使用工具函数进行清洗, 保存到tmp_pair列表中
            tmp_pair.append(normalizeString(item))      # normalizeString()  方法为自定义的字符串处理的方法，规范化字符串
        # 将tmp_pair列表保存到my_pairs列表中
        my_pairs.append(tmp_pair)
        # 重置tmp_pair列表
        tmp_pair = []
    # print('my_paris的长度为--->', len(my_pairs))
    # print('my_paris[:4]--->', my_pairs[:4])           # 获取前四条数据
    ...
    # print('english_word_n--->', english_word_n)           # 英语词表个数
    # print('french_word_n--->', french_word_n)             # 法语词表个数
...
class MyPairsDataset(Dataset):
    ...
    def __init__(self, my_pairs, english_word2index, french_word2index):        # 传入 处理后的样本，英语词表，法语词表
    ...
    def __getitem__(self, index):
        ...
        # tensor_x 和 tensor_y 是标量张量，只用每个单词的的索引
...
if __name__ == '__main__':
    ...
    # print(type(my_dataloader))          # <class 'torch.utils.data.dataloader.DataLoader'>
    # print(my_dataloader)           # <torch.utils.data.dataloader.DataLoader object at 0x0000023FFFF8EFD0>

    # 循环遍历数据加载器
    for i, (x, y) in enumerate(my_dataloader):          # 遍历的是打乱后的数据

...
if __name__ == '__main__':
    ...
    for i, (x, y) in enumerate(my_dataloader):
        ...
        # print("x：", x)		# 样本中的每一个英语样本句子，用于放入到编码器中
        print('x.shape ---> ', x.shape)  # batch_size、句子数, seq_len
        # 编码器进行编码 一次性喂数据
        # 初始化隐藏状态值
        hidden = my_encoderrnn.inithidden()
        encoder_output, hn = my_encoderrnn(x, hidden)  # 编码器返回的是output 和 最后一个时间步的隐藏状态
        # print('encoder_output ---> ', encoder_output)			# 编码器的output输出，即：h1, h2, ... , hn
        print('encoder_output.shape ---> ', encoder_output.shape)  # batch_size、句子数, seq_len、句子长度, hidden_size
        # print('hn--->', hn)		# 编码器的最后一个时间步的隐藏状态
        print('hn.shape--->', hn.shape)  # num_layer, batch_size, hidden_size       # hn.shape---> torch.Size([1, 1, 256])
        ...
        for idx in range(x.shape[1]):  # 遍历英语句子样本的每一个词
            # print("encoder_output_c：", encoder_output_c[idx])
            # print("encoder_output_c.shape：", encoder_output_c[idx].shape)  # torch.Size([256])
            # print("encoder_output[0]：", encoder_output[0][idx])
            # print("encoder_output[0].shape：", encoder_output[0][idx].shape)  # torch.Size([256])
            encoder_output_c[idx] = encoder_output[0][idx]  # 将句子的每一个词(TOKEN)的词向量赋值到新的张量中
        # print('encoder_output_c--->', encoder_output_c.shape, encoder_output_c)

        # 解码器进行解码, 自回归, 一个一个token进行解码
        print("y.shape：", y.shape)  # torch.Size([1, 8])     (batch_size、句子数, seq_len)
        for j in range(y.shape[1]):  # 遍历法语句子样本中的每一个词

            # 获取当前预测token时间步的输入x(等同于上一时间步的预测y)
            # 当前以真实y中的每个token作为输入, 模拟解码器的界面过程, 实际上第一个输入token一定是起始符号
            tmp_y = y[0][j].view(1, -1)  # 将每一个词的标量转化为二维词向量.
            print("y[0][j].shape：", y[0][j].shape)  # torch.Size([])
            print("tmp_y.shape：", tmp_y.shape)  # torch.Size([1, 1])

            # 进行解码
            # 初始的隐藏状态值 = 编码器最后一个时间步的隐藏状态值
            # output, hidden = my_decoderrnn(tmp_y, hn)		# 没加注意力机制的解码器

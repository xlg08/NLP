...
def train_rnn():
    ...
    # rnn = RNN(input_size=n_letters, hidden_size=128, output_size=n_categories).to(device)
    ...
    # 5.损失
    ce_loss = NLLLoss()             # 初始化损失函数
    starttime = time.time()         # 记录训练开始的时间
    ...
    for epoch in range(epochs):
        ...
        for i, (x_batch, y_batch) in enumerate(dataloader):
            ...
            # 8.模型预测
            # h0 = rnn.initHidden(x_batch).to(device)
            ...
            total_iter_num = total_iter_num + x_batch.shape[0]      # 计算该轮次的样本总数

            # total_loss = total_loss + loss.item()
            total_loss += loss.item()       # 计算损失和
            ...
            if (total_iter_num % 10 == 0):              # 如果训练了10个样本了
                # 平均损失
                tmploss = total_loss / total_iter_num       # 计算平均损失
                total_loss_list.append(tmploss)         # 平均损失放到平均损失列表中
                # 平均准确率
                tmpacc = total_acc_num / total_iter_num     # 计算平均准确率
                total_acc_list.append(tmpacc)           # 平均准确率放到平均准确率列表中

            if (total_iter_num % 20 == 0):          # 如果训练了20个样本了，计算平均损失、平均准确率，输出到控制台中
                tmploss = total_loss / total_iter_num           # 计算平均损失
                tmpacc = total_acc_num / total_iter_num         # 计算平均准确率
                print('轮次:%d, 平均损失:%.6f, 时间:%d，准确率:%.3f' % (
                    epoch + 1, tmploss, time.time() - starttime, tmpacc))
            # todo:6-3每个轮次保存模型

    # 13.模型保存
    torch.save(rnn.state_dict(), 'weight/rnn.pt')
    total_time = time.time() - starttime        # 计算该模型训练的总时间
    return total_loss_list, total_acc_list, total_time      # 返回平均损失、平均准确率、训练总时间
...
def train():
    ...
    # legend 根据label自动添加图例
    # 参数说明（常见）：
    #   loc：图例的位置，比如 'upper right', 'lower left', 'best' 等。
    #   fontsize：图例文字的字号。
    #   title：图例的标题。
    #   frameon：是否绘制图例外框（布尔值）。
    plt.legend()
    plt.savefig('img/loss.png')  # 保存图片
    plt.show()
    ...
    plt.figure(2)
    # bar 柱状图
    plt.bar(range(3), [rnn_time, lstm_time, gru_time], tick_label=['rnn_time', 'lstm_time', 'gru_time'])
